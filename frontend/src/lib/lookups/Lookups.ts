...
export function setLookups(payload: LookupsPayload) {
	store.set(payload);
}
export function getLookups(): LookupsPayload {
	return get(store);
}
...
export function statusName(id: TicketStatus) {
	return get(store).ticketStatus.find((x) => x.id === id)?.name;
}
export function priorityName(id: Priority) {
	return get(store).priority.find((x) => x.id === id)?.name;
}
// Category name is now fetched from categories store (see getCategoryName in $lib/stores/categories)
export function roleName(id: UserRole) {
	return get(store).userRole.find((x) => x.id === id)?.name;
}
