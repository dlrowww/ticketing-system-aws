...
export async function fetchTickets(query: TicketQuery, fetchFn: typeof fetch = fetch, signal?: AbortSignal): Promise<PagedResult<TicketListItem>> {
...
export async function exportTicketsCsv(query: TicketQuery, fetchFn: typeof fetch = fetch, signal?: AbortSignal): Promise<Blob> {
  const params = new URLSearchParams();
...
export async function createTicket(form: FormData, fetchFn: typeof fetch = fetch, signal?: AbortSignal): Promise<CreateTicketResponse> {
  ...
  if (!res.ok) {
    ...
    throw createApiError(`Create ticket failed: ${res.status} ${res.statusText}`, { code, traceId, detail: text });
  }

  return (await res.json()) as CreateTicketResponse;
}

export async function getTicketById(id: number, fetchFn: typeof fetch = fetch, signal?: AbortSignal) {
...
export async function updateTicket(
  ...
  if (res.status === 400) {
    ...
    if (isValidationProblemDetailsPayload(payload) && payload.errors && Object.keys(payload.errors).length > 0) {
  ...
  if (!res.ok) {
    ...
    const traceId = payload?.traceId;
    const text = payload?.detail ?? (await res.text().catch(() => ''));
    throw createApiError(`Update ticket failed: ${res.status} ${res.statusText}`, { code, traceId, detail: text });
  }
...
export function ticketFileDownloadUrl(ticketId: number, fileId: number, downloadRoute?: string | null, inline = false) {
