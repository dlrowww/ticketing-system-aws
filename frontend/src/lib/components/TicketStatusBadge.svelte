...
<style>
  /* Neutral / incoming */
  :global(.bg-status-new)        { background: #f1f3f5; color: #495057; } /* gray */

  /* Active / backlog (not danger) */
  :global(.bg-status-open)       { background: #e7f5ff; color: #1c7ed6; } /* blue */

  /* Work in progress */
  :global(.bg-status-inprocess)  { background: #fff9db; color: #f59f00; } /* yellow */

  /* Done / successful */
  :global(.bg-status-resolved)   { background: #ebfbee; color: #2f9e44; } /* green */

  /* Terminated / negative */
  :global(.bg-status-cancelled)  { background: #ffe3e3; color: #e03131; } /* red */

  /* Waiting / paused (blocked) */
  :global(.bg-status-postponed)  { background: #fff3bf; color: #f08c00; } /* amber */

  /* Returned / rework (quality signal), distinct from cancelled but still “attention” */
  :global(.bg-status-returned)   { background: #f3f0ff; color: #6741d9; } /* violet */

  /* Fallback */
  :global(.bg-status-unknown)    { background: #f1f3f5; color: #6c757d; } /* gray */
</style>