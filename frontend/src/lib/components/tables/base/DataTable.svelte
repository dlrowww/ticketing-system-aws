					...
					{#each Array(pagination?.pageSize ?? 10) as _}
						<tr>
							{#if config.enableSelection}
								<td style="width: 40px;"
									><SkeletonLoader type="rect" width="16px" height="16px" /></td
								>
							{/if}
							{#each config.columns as column}
								<td>
							...
							{#if (config.actions?.length ?? 0) > 0}
								<td class="text-center"
									><SkeletonLoader type="rect" width="60px" height="28px" /></td
								>
							{/if}
						</tr>
					{/each}
							...
							{#if config.enableSelection ?? true}
								<td>
									<input
										...
										onchange={(e) =>
											toggleRow(row[config.keyField], (e.target as HTMLInputElement).checked)}
									...
									{:else if column.formatter}
										...
										{#if typeof formatted === 'string'}
											...
											{@const translatedValue =
												formatted.startsWith('file_type_') || formatted.startsWith('category_')
									...
									{#each config.actions ?? [] as action}
										...
										{#if shouldShow}
											...
											{@const toggleTitle = isActive
												? 'action_deactivate_category'
												: 'action_activate_category'}
											<button
