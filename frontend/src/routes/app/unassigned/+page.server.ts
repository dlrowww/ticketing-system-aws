...
export const load: PageServerLoad = async ({ fetch, url, parent }) => {
	...
	if (!Number.isFinite(roleId) || roleId !== UserRole.Support) {
		throw redirect(303, '/app/my-tickets');
	}

	const categoryId = Number(user?.categoryId);
	if (!Number.isFinite(categoryId)) {
		const params = buildTicketsSearchParams(url, { assignedToUserId: null });
		return {
	...
	return {
		...
		...(await loadTicketsListPage({
			fetch,
			url,
			fixed: { assignedToUserId: null, category: categoryId }
		}))
	};
};
