...
describe('Dashboard (Charts) +page.server load', () => {
	...
	it('loads report data for Admin and clamps days to 365', async () => {
		...
		expect(reports.fetchDashboardStats).toHaveBeenCalledWith(
			{ from: '2025-01-01', to: '2025-01-31' },
			expect.any(Function)
		);
		expect(reports.fetchTicketsByCategory).toHaveBeenCalledWith(
			{ from: '2025-01-01', to: '2025-01-31' },
			expect.any(Function)
		);
		expect(reports.fetchTicketsByStatus).toHaveBeenCalledWith(
			{ from: '2025-01-01', to: '2025-01-31' },
			expect.any(Function)
		);
